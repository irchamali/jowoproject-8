...
class Edu extends CI_Controller
{
	...
	function index()
	{
		...
		$config['base_url'] = base_url() . 'edu/page/';
		...
		$x['data'] = $this->edu_model->get_edu_perpage($offset, $limit);
		//print_r($this->db->last_query()); 
		$x['judul'] = "Edu";
		if (empty($this->uri->segment(3))) {
			$next_page = 2;
			$x['canonical'] = site_url('edu');
			$x['url_prev'] = "";
		} elseif ($this->uri->segment(3) == '1') {
			$next_page = 2;
			$x['canonical'] = site_url('edu');
			$x['url_prev'] = site_url('edu');
		} elseif ($this->uri->segment(3) == '2') {
			$next_page = $this->uri->segment(3) + 1;
			$x['canonical'] = site_url('edu/page/' . $this->uri->segment(3));
			$x['url_prev'] = site_url('edu');
		} else {
			$next_page = $this->uri->segment(3) + 1;
			$prev_page = $this->uri->segment(3) - 1;
			$x['canonical'] = site_url('edu/page/' . $this->uri->segment(3));
			$x['url_prev'] = site_url('edu/page/' . $prev_page);
		}

		$x['url_next'] = site_url('edu/page/' . $next_page);
		...
		$this->load->view('edu_view', $x);
	}


	function detail($slug)
	{
		$data = $this->edu_model->get_post_by_slug($slug);
		if ($data->num_rows() > 0) {
			$q = $data->row_array();
			$kode = $q['post_id'];
			...
			$this->edu_model->count_views($kode);
			$x['related_post']  = $this->edu_model->get_related_post($category_id, $kode);
			$x['show_comments'] = $this->edu_model->show_comments($kode);
			$site_info = $this->db->get('tbl_site', 1)->row();
			$v['logo'] =  $site_info->site_logo_header;
			$x['icon'] = $site_info->site_favicon;
			$x['header'] = $this->load->view('header', $v, TRUE);
			...
			$this->load->view('edu_detail_view', $x);
		} else {
			redirect('edu');
		}
	}
	...
	function submit_komentar()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->edu_model->save_comment($post_id, $name, $email, $comment);
