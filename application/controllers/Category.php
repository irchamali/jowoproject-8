...
class Category extends CI_Controller
{
	...
	{
		$data = $this->category_model->get_edu_by_category($slug);
		if ($data->num_rows() > 0) {
			$q = $data->row_array();
			...
			$x['data'] = $this->category_model->edu_category_perpage($category_id, $offset, $limit);
			...
			$this->load->view('edu_category_view', $x);
		} else {
			redirect('edu');
		}
	}
